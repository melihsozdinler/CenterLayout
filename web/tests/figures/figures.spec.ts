...
/**
 * The descent as a LaTeX table body, so the manuscript inputs measured numbers rather
 * than transcribed ones. Transcription is where papers acquire numbers that were true
 * once.
 */
function descentTable(levels: readonly DescentLevel[]): string {
  const rows = levels.map((level, depth) => {
    ...
    return (
      `${depth} & ${level.order.toLocaleString('en-US')} & ` +
      `${level.size.toLocaleString('en-US')} & ${grouping} & ${level.modules} & ` +
      `${level.largest.toLocaleString('en-US')} & ${level.seconds.toFixed(1)} \\\\`
    )
  })
  ...
  return (
    ...
    `${rows.join('\n')}\n` +
    (last
      ? `\\multicolumn{7}{l}{\\footnotesize After the last step, ` +
        `${last.largest.toLocaleString('en-US')} proteins remain --- ` +
        `drawn as proteins.} \\\\\n`
      : '')
  )
}
