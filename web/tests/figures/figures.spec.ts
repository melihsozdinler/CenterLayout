...
test('generate paper figures', async ({ page }) => {
  ...
  // The protein network itself, under the two arrangements that answer different
  // questions. Fifty proteins rather than the organism: what an arrangement means is
  // legible at that size, and at eight thousand a figure says only that it is large.
  const networks = await page.evaluate(async (id) => {
    const graph = (
      await window.prolivis!.graph(
        { datasetId: id, organismId: 2697049, physicalOnly: true, excludeSelfInteractions: true },
        { minScore: 0.4 },
      )
    ).reduce({ minDegree: 2, maxEdges: 400 })

    const draw = (mode: 'force' | 'grouped') => {
      const layout = window.prolivis!.networkLayout(graph, { mode })
      return {
        svg: window.prolivis!.toSvg(
          window.prolivis!.networkScene(layout, { colourBy: 'module', labelBelow: 200 }),
          `SARS-CoV-2 network, ${mode} layout`,
        ),
        groups: layout.groupCount,
      }
    }
    const force = draw('force')
    const grouped = draw('grouped')
    return {
      proteins: graph.order,
      interactions: graph.size,
      communities: grouped.groups,
      force: force.svg,
      grouped: grouped.svg,
    }
  }, dataset.datasetId)

  writeFileSync(resolve(OUTPUT, 'network-force.svg'), networks.force, 'utf8')
  writeFileSync(resolve(OUTPUT, 'network-grouped.svg'), networks.grouped, 'utf8')
  writeFileSync(
    resolve(OUTPUT, 'numbers-network.tex'),
    [
      '% Generated by tests/figures/figures.spec.ts. Do not edit.',
      `\\newcommand{\\NetProteins}{${networks.proteins}}`,
      `\\newcommand{\\NetInteractions}{${networks.interactions}}`,
      `\\newcommand{\\NetCommunities}{${networks.communities}}`,
      '',
    ].join('\n'),
    'utf8',
  )
  console.log(
    `  network figures: ${networks.proteins} proteins, ${networks.interactions} ` +
      `interactions, ${networks.communities} communities`,
  )
